"""Revenue, top items, tips, peak hours."""
...
st.subheader("📦 Low stock right now")
low_stock = query("select * from inventory where quantity_on_hand <= reorder_level order by item_name")
if low_stock:
    for it in low_stock:
        st.write(f"⚠️ **{it['item_name']}** — {it['quantity_on_hand']} {it['unit']} left")
else:
    st.caption("All stock above reorder levels.")
