"""Mark orders paid, capture tips per waiter."""
...
if not served_orders:
    ...
    for order in served_orders:
        with st.container(border=True):
            ...
            st.subheader(f"{order['order_number']} — {label}")
            ...
            if st.button(f"Confirm Payment — {format_ugx(total)}", key=f"confirm_{order['id']}", type="primary"):
                ...
                if tip > 0 and staff_options.get(waiter_name):
                    execute(
                        "insert into tips (order_id, staff_id, amount, method) values (%s, %s, %s, %s)",
...
st.divider()
st.subheader("📊 Tips summary — all staff (this week)")
tip_summary = query(
